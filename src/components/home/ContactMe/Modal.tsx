...
const ContactMeModal: FC<ContactMeModalProps> = ({ onClose }) => {
	...
	return (
		...
		>
			...
			>
				<img src={contactMeBg} alt="" className="w-1/2 h-full object-cover max-lg:hidden" />

				<form className="w-1/2 max-lg:w-full flex flex-col gap-10 px-10 py-10 justify-center">
					<div className="flex flex-col gap-4">
						<h1 className="text-2xl font-semibold max-lg:text-lg max-sm:flex gap-10"><button className="text-sm hidden max-sm:block" type="button" onClick={() => setIsClosing(true)}><IoMdArrowBack /></button> Começar um projecto</h1>
						<p>Insira suas informações e mande sua sugestão.</p>
					</div>
					...
					<div className="flex flex-col w-full items-center gap-5">
						<div className="flex flex-col w-full items-start gap-2">
							...
							<input
								...
								name=""
								id=""
								className="h-8 w-full rounded-md outline-none p-2 text-gray-500"
							/>
						</div>
						...
						<div className="flex flex-col w-full items-start gap-2">
							...
							<textarea
								name=""
								id=""
								className="w-full rounded-md outline-none p-2 text-gray-500 resize-none"
								cols={20}
								rows={4}
							/>
						</div>
